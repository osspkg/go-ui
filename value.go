...
var expressionArity = map[string][2]int{
	"$eq": {2, 2}, "$ne": {2, 2}, "$gt": {2, 2}, "$gte": {2, 2},
	"$lt": {2, 2}, "$lte": {2, 2}, "$and": {1, -1}, "$or": {1, -1},
	"$not": {1, 1}, "$in": {2, 2}, "$exists": {1, 1}, "$concat": {0, -1},
	"$coalesce": {1, -1}, "$if": {3, 3},
}

func Literal(data any) Value       { return Value{Kind: ValueLiteral, Data: data} }
func StateRef(path string) Value   { return Value{Kind: ValueState, Path: path} }
func ContextRef(path string) Value { return Value{Kind: ValueContext, Path: path} }
func SourceRef(path string) Value  { return Value{Kind: ValueSource, Path: path} }
func EventRef(path string) Value   { return Value{Kind: ValueEvent, Path: path} }
func ResultRef(path string) Value  { return Value{Kind: ValueResult, Path: path} }
