...
func DefaultLimits() Limits {
	return Limits{
		MaxBytes: 1 << 20,
		MaxNodes: 1000, MaxDepth: 32, MaxPropsPerNode: 64,
		MaxActions: 128, MaxSources: 64, MaxStrings: 10000, MaxStringLength: 16 << 10,
		MaxExpressionDepth: 32,
	}
}

func (v ViewSchema) Validate() error { return v.ValidateWithLimits(DefaultLimits()) }
...
func (v ViewSchema) ValidateWithLimits(limits Limits) error {
	...
	if v.Regions.TopHeader == nil || v.Regions.LeftPanel == nil || v.Regions.Content == nil || v.Regions.RightPanel == nil || v.Regions.Bottom == nil {
	...
	for region, list := range map[string][]Node{
		"top-header": v.Regions.TopHeader, "left-panel": v.Regions.LeftPanel,
		"content": v.Regions.Content, "right-panel": v.Regions.RightPanel,
		"bottom": v.Regions.Bottom,
	} {
		for _, node := range list {
			if err := validateNode(node, region, 1, &nodes, seen, limits, v); err != nil {
				return err
			}
		}
	}
...
func validateSerializedLimits(schema ViewSchema, limits Limits) error {
	...
	strings := 0
	if err := walkLimitValue(document, 0, limits, &strings); err != nil {
		return fmt.Errorf("%w: %w", ErrInvalidSchema, err)
	}
	return nil
}

func walkLimitValue(value any, depth int, limits Limits, strings *int) error {
	...
	case []any:
		for _, item := range typed {
			if err := walkLimitValue(item, depth+1, limits, strings); err != nil {
				return err
			}
		}
	case map[string]any:
		for key, item := range typed {
			if err := walkLimitValue(key, depth+1, limits, strings); err != nil {
				return err
			}
			if err := walkLimitValue(item, depth+1, limits, strings); err != nil {
				return err
...
func validateNode(node Node, region string, depth int, nodes *int, seen map[string]struct{}, limits Limits, view ViewSchema) error {
	*nodes++
...
func validateValue(value Value, limits Limits, view ViewSchema, depth int) error {
	...
	case ValueExpr:
		...
		if len(args) < arity[0] || (arity[1] >= 0 && len(args) > arity[1]) {
			return fmt.Errorf("%w: operator %s expects %d..%d arguments", ErrInvalidValue, value.Path, arity[0], arity[1])
		}
...
func validEffectType(value string) bool {
	switch value {
	case "set-state", "merge-state", "invalidate", "refresh-source", "refresh-view", "patch-view", "navigate", "toast", "dialog", "close-dialog":
		return true
	default:
		return false
	}
